...
function validatePrivateKey(key: string): { valid: boolean; error?: string } {
  if (key.includes("BEGIN RSA PRIVATE KEY")) {
    return {
      valid: false,
      error: "Unsupported key type: RSA (PKCS#1). Generate a new JSON key (PKCS#8 BEGIN PRIVATE KEY)."
    };
  }
  ...
  if (!key.includes("BEGIN PRIVATE KEY") || !key.includes("END PRIVATE KEY")) {
    return {
      valid: false,
      error: "Invalid private key format. Use BASE64 env or ensure escaped newlines (\\n) with BEGIN/END PRIVATE KEY."
    };
  }
...
export async function GET() {
  try {
    ...
    if (!privateKey) missing.push("GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY or GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY_BASE64");
    ...
    const sheetName = env.GOOGLE_SHEETS_WORKSHEET_NAME || 
